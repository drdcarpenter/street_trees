---
title: "Hello, I'm Dan Carpenter"
image: "dc_temp.jpg"
# links:
#   - LinkedIn: "https://www.linkedin.com/in/dan-carpenter-cenv-057b512a/"
#   - Twitter: "https://twitter.com/drdcarpenter"
#   - GitHub: "https://github.com/drdcarpenter"
output:
  postcards::trestles
---
 
## My Apps

Here are some apps I have made using Shiny and R.

<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<style>
.btn {
  background-color: DarkGrey;
  border: none;
  color: white;
  padding: 12px 16px;
  text decoration: none;
  font-size: 16px;
  cursor: pointer;
}

/* Darker background on mouse-over */
.btn:hover {
  background-color: Green;
}

.button2 {border-radius: 4px;}

</style>

### Street tree app
A Shiny application to explore the benefits we gain from street tree planting.
<a href="/street_trees/" target="_blank"><button class="btn button2"><i class="fa fa-tree"></i> Street Tree app</button></a> 
